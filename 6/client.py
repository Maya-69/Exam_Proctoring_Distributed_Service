...
# Student data from config - PRESERVED FROM BASE
student_list = config.get('student_list', [29, 40, 42, 50, 52])
student_status = {i: 0 for i in student_list}
test_duration_sec = config.get('test_duration_sec', 30)
...
# TASK 6: NEW Interactive Exam Functions with Deadlock Handling
def start_interactive_exam(student_id):
    """Start interactive exam with 5-minute timer and deadlock handling"""
    ...
    try:
        ...
        print(f"TIMER: 5 minutes (300 seconds)")
        ...
        if result and result.get("success"):
            ...
            print(f"Duration: {result.get('duration', 300)} seconds")
...
def handle_exam_timeout(student_id):
    ...
    print(f"\n" + "="*80)
    print(f"AUTO-SUBMISSION COMPLETE!")
    print(f"Your partial answers have been processed")
...
def conduct_interactive_exam(student_id):
    ...
    print(f"Student: {student_id} ({students_info.get(student_id, 'Unknown')})")
    print(f"AUTO-SUBMIT WARNING: Exam will auto-submit when timer expires!")
    print(f"="*90)
    ...
    while exam_active:
        try:
            ...
            if remaining_time <= 30:
                print(f"WARNING: TIME REMAINING: {remaining_time:.0f} seconds - HURRY UP!")
            else:
            ...
            while True:
                try:
                    if remaining_time <= 10:
                        print(f"WARNING: ONLY {remaining_time:.0f} SECONDS LEFT!")
            ...
            if answer_result.get("success"):
                ...
                print(f"Result: {'CORRECT!' if is_correct else 'Wrong'}")
                if remaining_time <= 30:
                    print(f"WARNING: Time remaining: {remaining_time:.0f} seconds - HURRY!")
                else:
                    print(f"Time remaining: {remaining_time:.0f} seconds")
                print(f"="*50)
    ...
    if not exam_active:
        ...
        print(f"Waiting for final results from teacher...")
        print(f"Marksheet will be displayed once all processing is complete")
        print(f"="*80)


def attempt_manual_submission_with_deadlock(student_id):
    """TASK 6: Manual submission with deadlock detection and resolution"""
    ...
    print(f"[CLIENT] This may trigger deadlock if auto-timer expires simultaneously...")
    print(f"="*80)
    ...
    try:
        ...
        if result and result.get("success"):
            if result.get("deadlock_resolved"):
                print(f"\n" + "="*70)
                print(f"DEADLOCK DETECTED AND RESOLVED!")
                print(f"Resolution Strategy: {result.get('resolution_strategy', 'Unknown')}")
                print(f"="*70)
                deadlock_detected = True
            
            print(f"\n" + "="*70)
            print(f"FINAL SUBMISSION SUCCESSFUL!")
            print(f"="*70)
            ...
            if submission_type == 'manual':
                print(f"SUCCESS: Manual submission completed!")
                print(f"You successfully submitted before the timer expired!")
            else:
                print(f"AUTO-SUBMISSION: Timer expired during manual attempt")
            print(f"="*70)
...
def receive_exam_results(student_id, score, submission_type):
    """Receive final exam results from teacher"""
    exam_results[student_id] = {"score": score, "type": submission_type}
    
    score_out_of_100 = score * 10
    
    print(f"\n" + "="*80)
    print(f"FINAL EXAM RESULTS RECEIVED FROM TEACHER")
    print(f"="*80)
    print(f"Student: {student_id} ({students_info.get(student_id, 'Unknown')})")
...
def run():
    ...
    print(f"[{get_formatted_time()}] Starting client with Task 6: Deadlock in Exam Grid...")
    ...
    print(f"[{get_formatted_time()}] [CLIENT-TASK6] Deadlock Exam System with 5-minute timer")
    ...
    # TASK 6: User choice between Traditional and Interactive
    print(f"\n" + "="*60)
    print(f"SELECT EXAMINATION MODE:")
    print(f"1. Traditional Cheating Detection (Berkeley + Ricart-Agrawala)")
    print(f"2. Interactive Deadlock Exam (Task 6 - 5 minute MCQ with deadlock handling)")
    print(f"="*60)

...
test_duration_sec = config.get('test_duration_sec', 20)
...
number_of_request_for_bombardement = 30
...
def display_load_test_results():
    ...
    if total_requests_sent > 0:
        ...
        if success_rate >= 100:
            print(f"[{get_formatted_time()}] RESULT: PERFECT - 100% Success Rate!")
            print(f"[{get_formatted_time()}] No service denial - All requests processed")
        elif success_rate >= 95:
            print(f"[{get_formatted_time()}] RESULT: EXCELLENT - High success rate")
        else:
            print(f"[{get_formatted_time()}] RESULT: NEEDS IMPROVEMENT - Some requests failed")
...
def start_interactive_exam(student_id):
    ...
    try:
        ...
        print(f"TIMER: 2 minutes (120 seconds)")
        ...
        if result and result.get("success"):
            ...
            print(f"Duration: {result.get('duration', 120)} seconds")
...
def run():
    ...
    print(f"[{get_formatted_time()}] [CLIENT-TASK7] Server Load Balancing Test (Transparent to Client)")
    ...
    if choice == '2':
        ...
        bombard_server_with_requests()  # This now handles everything automatically
    else:

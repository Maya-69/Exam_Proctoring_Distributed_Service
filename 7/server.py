...
BACKUP_PORT = 8001
main_server_buffer = queue.Queue(maxsize=8)  # Smaller buffer for faster backup trigger
backup_server_buffer = queue.Queue(maxsize=8)
load_threshold = 5  # Lower threshold - backup triggers at 5/8 = 62.5%
...
def process_exam_on_backup_direct(student_id):
    """Process directly on backup without external RPC call"""
...
def process_exam_on_main(student_id):
    ...
    try:
        ...
        try:
            ...
            # If main buffer full, try backup immediately
            print(f"[{get_formatted_time()}] [SERVER-MAIN] Buffer full, redirecting to backup")
            del main_exam_sessions[student_id]  # Remove from main
            main_server_processed -= 1  # Adjust counter
            return process_exam_on_backup_direct(student_id)
...
def run_server():
    ...
    print(f"[{get_formatted_time()}] [SERVER] Features: Berkeley, Ricart-Agrawala, Task 6 Deadlock, Task 7 Load Balancing")
    ...
    print(f"[{get_formatted_time()}] [SERVER] Buffer: {main_server_buffer.maxsize} slots, Threshold: {load_threshold} ({(load_threshold/main_server_buffer.maxsize)*100:.0f}%)")
    print(f"[{get_formatted_time()}] [SERVER] 100% Success Rate Target - No Failed Requests")
    ...
    print(f"[{get_formatted_time()}] [SERVER] Ready to handle requests with optimized load balancing...")
